...
class VietstockCrawler:
    ...
    def _extract_article_details(self, session: requests.Session, link: str, title: str = "") -> Dict[str, Any]:
        with self._cache_lock:
            if link in self._cache:
                cached = self._cache[link].copy()
                if cached.get("content") or cached.get("is_pdf"):
                    return cached
        ...
        # Fallback if content is completely empty for non-pdf
        if not is_pdf and not content and title:
            content = title

        res = {"date": date, "content": content, "pdf_link": pdf_link, "is_pdf": is_pdf}
        with self._cache_lock:
            self._cache[link] = res
        return res
            ...
            try:
                ...
                for r in rows:
                    ...
                    if a and a.has_attr("articleid"):
                        ...
                        details = self._extract_article_details(session, link, title=title)
                        results.append({"code": stock_code, "id": aid, "title": title, "link": link, **details})
                time.sleep(0.2)
            except Exception as e:
