...
class NewsArticleProcessor:
    ...
    def process_pdf_article(self, article: Dict[str, Any], fetch_fn: Optional[Callable] = None, convert_fn: Optional[Callable] = None, analyze_text_fn: Optional[Callable] = None, normalize_fn: Optional[Callable] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not md:
                logger.warning("DataLab OCR conversion unavailable for PDF %s; skipping", article.get("id"))
                return None

            logger.info("✓ DataLab converted PDF %s to Markdown (%d chars)", article.get("id"), len(md))
            score = normalize(analyze_text(md, article.get("code", "")))
            return {"article_id": article["id"], "stock_code": article["code"], "title": article.get("title", ""), "link": article.get("link", ""), "date": datetime.now(), "is_pdf": True, "content": md, "pdf_link": pdf_link, "sentiment_score": score, "crawled_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        except Exception as e:
    ...
    def process_text_article(self, article: Dict[str, Any], analyze_text_fn: Optional[Callable] = None, normalize_fn: Optional[Callable] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"article_id": article["id"], "stock_code": article["code"], "title": article.get("title", ""), "link": article.get("link", ""), "date": datetime.now(), "is_pdf": False, "content": content, "pdf_link": None, "sentiment_score": score, "crawled_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        except Exception as e:
        ...
        if pdf_articles:
            with ThreadPoolExecutor(max_workers=self.pdf_max_workers) as ex:
                ...
                for f in as_completed(futures):
                    try:
                        res = f.result() or text_proc(futures[f])
