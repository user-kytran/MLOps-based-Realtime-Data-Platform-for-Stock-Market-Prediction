...
impl Consumer for PrinterConsumer {
    async fn consume_cdc(&mut self, data: CDCRow<'_>) -> anyhow::Result<()> {
        ...
        row_to_print.push_str(&print_row_change_header(&data, processing_time));
...
fn print_row_change_header(data: &CDCRow<'_>, processing_time: i64) -> String {
    ...
    let producer_timestamp = match data.get_value("producer_timestamp") {
        Some(v) => v.as_bigint().unwrap_or(0),
        None => 0,
    };
    
    let latency_ms = if producer_timestamp > 0 {
        processing_time - producer_timestamp
    } else {
        -1
    };
    ...
    header_to_print.push_str(&print_field("⚡ LATENCY:", &latency_str));
    header_to_print.push_str(
        "├────────────────────────────────────────────────────────────────────────────┤\n",
    );
    header_to_print
